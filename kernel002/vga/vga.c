...
void vga_init()
{
    ...
    for (vga_char = vga_base; vga_char < vga_bound; vga_char++) //vga_char++, 每次值增大2
...
void vga_put_char(char c)
{
    ...
    row = (vga_cursor - vga_base) / vga_column_number; //cursor - vga_base, 步数的差值, 不是地址差值
    ...
    case '\n':
        row++;
        column = 0;
        vga_cursor = vga_base + (row * vga_column_number);
        break;
